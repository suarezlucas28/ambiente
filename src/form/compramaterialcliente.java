 ...
 * @author Luke
 */
public class compramaterialcliente extends javax.swing.JPanel {
...
@Override
    ...
    public compramaterialcliente() throws ClassNotFoundException, InstantiationException, IllegalAccessException, SQLException {
        initComponents();
    ...
    private void gencod() {
        try {
            ...
            conn.resultado = conn.sentencia.executeQuery("SELECT IFNULL(MAX(com_codigo),0)+1 AS xxx FROM compras");
            conn.resultado.next();
    ...
    private void CabDetalle() {
        ...
        grilla.getColumnModel().getColumn(3).setPreferredWidth(5);
    ...
    public void grilla3() {
        ...
        try {
            ver_conex conn2 = new ver_conex();//instanciamos
            snt = conn2.conexion.createStatement();
            recur1 = snt.executeQuery("SELECT com_codigo,pro_nombre,CONCAT(cli_nomape,' - ',obr_proyec),com_fechor FROM compras,obras,proveedor,cliente WHERE compras.pro_codigo = proveedor.pro_codigo AND compras.obr_codigo = obras.obr_codigo AND obras.cli_codigo = cliente.cli_codigo AND CONCAT(cli_nomape,' - ',obr_proyec) LIKE '%" + wqqqq + "%' AND com_estado LIKE 'ACTIVO' ORDER BY com_codigo DESC");
            System.out.println(recur1);
            if (recur1.wasNull()) {
                ab = 0;
                return;
    ...
    private void agregar() throws SQLException {
        Fecha("Fecha");
        String SQL = "INSERT INTO compras (com_codigo,pro_codigo,obr_codigo,usu_codigo,com_fechor) "
                + " VALUES (" + lblcodigo.getText() + "," + proveedor + "," + txt_item_cod.getText() + "," + acceso.usuario + ",'" + fechahora + "')";
        ...
        for (int i = 0; i < model.getRowCount(); i++) {
                ...
                String SQL1 = "INSERT INTO compras_detalle (com_codigo,ite_codigo,com_cantid) VALUES (" + lblcodigo.getText() + "," + codigo + "," + cantidad + ")";
                conn.sentencia = conn.conexion.createStatement();
...
//        conn.sentencia.executeUpdate(SQL2);//OJO LE PASO LA SENTENCIA
    ...
    private void modificar() throws SQLException {
        Fecha("Fecha");
        String SQL = "UPDATE compras SET pro_codigo = " + proveedor + ",obr_codigo = " + txt_item_cod.getText() + ",usu_codigo = " + acceso.usuario + ",com_fechor = '" + fechahora + "' WHERE com_codigo = " + lblcodigo.getText();
//        String SQL2 = "UPDATE movimientos SET cue_monto = " + txttotal.getText() + ",cue_fechor = '" + fechahora + "' WHERE cue_segun = 'ENTREGA " + lblcodigo.getText() + "'";
        ...
        conn.sentencia = conn.conexion.createStatement();
//        conn.sentencia.executeUpdate(SQL2);//OJO LE PASO LA SENTENCIA
        String SQL1 = "DELETE FROM compras_detalle WHERE com_codigo =  " + lblcodigo.getText();
        conn.sentencia = conn.conexion.createStatement();
        conn.sentencia.executeUpdate(SQL1);
        try {
            javax.swing.table.TableModel model = grilla.getModel();
            ...
            for (int i = 0; i < model.getRowCount(); i++) {
                ...
                String cantidad = modelo.getValueAt(i, 3).toString();
                conn.sentencia = conn.conexion.createStatement();
                String insercionSQL1 = "INSERT INTO compras_detalle (com_codigo,ite_codigo,com_cantid) VALUES (" + lblcodigo.getText() + "," + codigo + "," + cantidad + ")";
    ...
    private void borrar() throws SQLException {
        String SQL = "UPDATE compras SET com_estado = 'ANULADO' WHERE com_codigo = " + lblcodigo.getText();
...
//        conn.sentencia.executeUpdate(SQL2);
    ...
    private void cargadatos() throws SQLException {
        ver_conex conn = new ver_conex();
        conn.sentencia = conn.conexion.createStatement();
        conn.resultado = conn.sentencia.executeQuery("SELECT * FROM compras WHERE com_codigo = " + lblcodigo.getText());
        conn.resultado.next();
        txthora.setText(conn.resultado.getString("com_fechor"));
        ...
        conn.sentencia = conn.conexion.createStatement();
        conn.resultado = conn.sentencia.executeQuery("SELECT compras_detalle.ite_codigo,ite_descri,uni_descri,com_cantid FROM unidades,compras_detalle,items WHERE items.ite_codigo = compras_detalle.ite_codigo AND items.uni_precom = unidades.uni_codigo AND com_codigo = " + lblcodigo.getText());
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(layout.createSequentialGroup()
                        ...
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            ...
                            .addGroup(layout.createSequentialGroup()
                                .addGap(117, 200, Short.MAX_VALUE)
...
//            operaciones();
            ...
            Logger.getLogger(compramaterialcliente.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void lblcodigoKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_lblcodigoKeyPressed
        if (evt.getKeyChar() == KeyEvent.VK_ENTER) {
            try {
                ver_conex conn = new ver_conex();
                conn.sentencia = conn.conexion.createStatement();
                conn.resultado = conn.sentencia.executeQuery("SELECT com_codigo,pro_nombre,CONCAT(cli_nomape,' - ',obr_proyec),com_fechor FROM compras,obras,proveedor,cliente WHERE compras.pro_codigo = proveedor.pro_codigo AND compras.obr_codigo = obras.obr_codigo AND obras.cli_codigo = cliente.cli_codigo AND com_estado LIKE 'ACTIVO' ORDER BY com_codigo DESC");
                modelo3.setRowCount(0);
                Object[] fila = new Object[4];
                while (conn.resultado.next()) {
                    fila[0] = conn.resultado.getObject(1);
                ...
                buscadorentrega.setTitle("COMPRA DE MATERIALES DEL CLIENTE");
    ...
    private void grilla3MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_grilla3MouseClicked
        try {
            ...
            if (operacion == 2) {
                ...
                int mensaje = JOptionPane.showConfirmDialog(this, "Deseas Anular Compra de Materiales N° --> " + lblcodigo.getText(), "Confirmar", JOptionPane.YES_NO_OPTION);
