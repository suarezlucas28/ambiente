...
public class infopamo1 extends javax.swing.JPanel {
    ...
    public infopamo1() throws ClassNotFoundException, InstantiationException, IllegalAccessException, SQLException {
        initComponents();
        des_op();
        des_btn();
    }
    ...
    try {
    if (op1.isSelected()){
    sentencia = "SELECT pagos1.`pag_codigo` AS pagos_pag_codigo,pag_concep AS obra,pagos1.`pag_horfec` AS pagos_pag_horfec,pagos1.`pag_monto` AS pagos_pag_monto,pagos1.`pag_destin` AS pagos_pag_destin,pagos1.`pag_observ` AS pagos_pag_observ,pagos1.`pag_fecrec` AS pagos_pag_fecrec,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `usuarios` usuarios INNER JOIN `pagos1` pagos1 ON usuarios.`usu_codigo` = pagos1.`usu_codigo` WHERE pag_estado LIKE 'ACTIVO' ORDER BY pag_codigo DESC";    
    } else if (op2.isSelected()){
    sentencia = "SELECT pagos1.`pag_codigo` AS pagos_pag_codigo,pag_concep AS obra,pagos1.`pag_horfec` AS pagos_pag_horfec,pagos1.`pag_monto` AS pagos_pag_monto,pagos1.`pag_destin` AS pagos_pag_destin,pagos1.`pag_observ` AS pagos_pag_observ,pagos1.`pag_fecrec` AS pagos_pag_fecrec,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `usuarios` usuarios INNER JOIN `pagos1` pagos1 ON usuarios.`usu_codigo` = pagos1.`usu_codigo` WHERE pag_estado LIKE 'ACTIVO' AND `pag_codigo` = "+txtcodigo.getText();
    } else if (op3.isSelected()){
    sentencia = "SELECT pagos1.`pag_codigo` AS pagos_pag_codigo,pag_concep AS obra,pagos1.`pag_horfec` AS pagos_pag_horfec,pagos1.`pag_monto` AS pagos_pag_monto,pagos1.`pag_destin` AS pagos_pag_destin,pagos1.`pag_observ` AS pagos_pag_observ,pagos1.`pag_fecrec` AS pagos_pag_fecrec,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `usuarios` usuarios INNER JOIN `pagos1` pagos1 ON usuarios.`usu_codigo` = pagos1.`usu_codigo` WHERE pag_estado LIKE 'ACTIVO' AND LEFT(`pag_horfec`,10) BETWEEN  '"+txtfdesde.getText()+"' AND '"+txtfhasta.getText()+"' order by pag_codigo desc";    
    } else if (op8.isSelected()){
    sentencia = "SELECT pagos1.`pag_codigo` AS pagos_pag_codigo,pag_concep AS obra,pagos1.`pag_horfec` AS pagos_pag_horfec,pagos1.`pag_monto` AS pagos_pag_monto,pagos1.`pag_destin` AS pagos_pag_destin,pagos1.`pag_observ` AS pagos_pag_observ,pagos1.`pag_fecrec` AS pagos_pag_fecrec,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `usuarios` usuarios INNER JOIN `pagos1` pagos1 ON usuarios.`usu_codigo` = pagos1.`usu_codigo` WHERE pag_estado LIKE 'ANULADO' ORDER BY pag_codigo DESC";    
...
op1.requestFocus();
    ...
    private void btngenerarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btngenerarActionPerformed
        try {
            ...
            Logger.getLogger(infopamo1.class.getName()).log(Level.SEVERE, null, ex);
        }
