...
public class cobros extends javax.swing.JPanel {
    ...
    public cobros() throws SQLException, ClassNotFoundException, InstantiationException, IllegalAccessException {
        initComponents();
    ...
    private void gencod() {
        try {
            ...
            conn.resultado = conn.sentencia.executeQuery("SELECT IFNULL(MAX(cob_codigo),0)+1 AS xxx FROM cobros");
            conn.resultado.next();
    ...
    private void grabar() throws SQLException {
        Fecha("Fecha");
        ver_conex conn = new ver_conex();
        String SQL = "INSERT INTO cobros (cob_codigo,usu_codigo,obr_codigo,cob_horfec,cob_monto,cob_numletra,cob_observ,cob_fecrec) VALUES ("+txtcodigo.getText()+","+acceso.usuario+","+txt_item_cod.getText()+",'"+fechahora+"',"+txtmontoacobrar.getText()+",'"+numletra+"','"+txtobserva.getText()+"','"+txtfdesde.getText()+"')";
        ...
        String SQL2 = "INSERT INTO movimientos (cue_codigo,usu_codigo,obr_codigo,cue_monto,cue_segun,cue_fechor,cue_opera)"
                + " VALUES (" + a + "," + acceso.usuario + "," + txt_item_cod.getText() + "," + txtmontoacobrar.getText() + ",'HONORARIOS " + txtcodigo.getText() + "','" + fechahora + "','RETIRO')";
    ...
    private void imprimir() throws SQLException, JRException {
        try {
            ...
            String sentencia = "SELECT cob_codigo,cob_monto AS monto,cob_numletra,CONCAT(cli_nomape,' - ',obr_proyec) AS obr_proyec,DAY(`cob_fecrec`) AS dia, MONTH(`cob_fecrec`) AS mes, YEAR(`cob_fecrec`) AS anno,cob_observ FROM cobros,obras,cliente WHERE cliente.cli_codigo = obras.cli_codigo AND obras.obr_codigo = cobros.obr_codigo AND cob_codigo = " + txtcodigo.getText();
            ////////////////////////////////instanciamos
        ...
        ////////////////////////////// preapra el patch
        URL urlMaestro = getClass().getClassLoader().getResource("repo/recibo2.jasper");
        ////////////////////////////// Cargamos el reporte
    ...
    public void grilla3() {
        ...
        try {
            ...
            recur1 = snt.executeQuery("SELECT cob_codigo,CONCAT(cli_nomape,' - ',obr_proyec) AS obra,cob_monto,cob_horfec FROM cobros,cliente,obras WHERE obras.cli_codigo = cliente.cli_codigo AND cobros.obr_codigo = obras.obr_codigo AND CONCAT(cli_nomape,' - ',obr_proyec) LIKE '%" + wqqqq + "%' AND cob_estado LIKE 'ACTIVO' ORDER BY cob_codigo DESC");
            if (recur1.wasNull()) {
                ab = 0;
                return;
            }
            ...
            if (recur1.next()) {
                do {
                    ab = recur1.getInt("cob_codigo");
                    ac = recur1.getString("obra");
                    ad = recur1.getString("cob_monto");
                    af = recur1.getString("cob_horfec");
                    modelo3.addRow(new Object[]{ab, ac, ad, af});
                } while (recur1.next());
            } else {
                ab = 0;
            }
    ...
    private void cabecerabuscador() {
        ...
        grilla4.getColumnModel().getColumn(3).setPreferredWidth(51);
    }

    private void cargadatos() throws SQLException {
        ver_conex conn = new ver_conex();
        conn.sentencia = conn.conexion.createStatement();
        conn.resultado = conn.sentencia.executeQuery("SELECT cob_codigo,obr_codigo,cob_monto,cob_fecrec FROM cobros,proveedor WHERE cob_codigo = " + txtcodigo.getText());
        conn.resultado.next();
        txtcodigo.setText(conn.resultado.getString("cob_codigo"));
        txt_item_cod.setText(conn.resultado.getString("obr_codigo"));
        txtmontoacobrar.setText(conn.resultado.getString("cob_monto"));
        txtfdesde.setText(conn.resultado.getString("cob_fecrec"));
        conn.sentencia = conn.conexion.createStatement();
        conn.resultado = conn.sentencia.executeQuery("SELECT IFNULL(SUM(cob_monto),0) AS PAGADO FROM cobros WHERE obr_codigo = " + txt_item_cod.getText() + " AND cob_estado LIKE 'ACTIVO'");
        conn.resultado.next();
    ...
    private void borrar() throws SQLException {
        String SQL;
        SQL = "UPDATE cobros SET cob_estado = 'ANULADO' WHERE cob_codigo = " + txtcodigo.getText();
        ver_conex conn = new ver_conex();//instanciamos
        conn.sentencia = conn.conexion.createStatement();
        conn.sentencia.executeUpdate(SQL);//OJO LE PASO LA SENTENCIA
        String SQL2 = "UPDATE movimientos SET cue_estado = 'ANULADO' WHERE cue_segun = 'HONORARIOS " + txtcodigo.getText() + "'";
        conn.sentencia = conn.conexion.createStatement();
    ...
    private void initComponents() {
        ...
        setMaximumSize(new java.awt.Dimension(585, 285));
        setMinimumSize(new java.awt.Dimension(585, 285));
        setPreferredSize(new java.awt.Dimension(585, 285));
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                        ...
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            ...
                            .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                                .addGroup(layout.createSequentialGroup()
                                    ...
                                    .addGap(18, 18, 18)
                                    .addComponent(txtmontoacobrar1, javax.swing.GroupLayout.PREFERRED_SIZE, 95, javax.swing.GroupLayout.PREFERRED_SIZE))
                                .addGroup(layout.createSequentialGroup()
                    ...
                    .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, layout.createSequentialGroup()
                        ...
                        .addGap(38, 38, 38)
                        .addComponent(txtfdesde)
                        .addGap(18, 18, 18)
                        .addComponent(jLabel8)
                        .addGap(18, 18, 18)
                        .addComponent(txtobserva, javax.swing.GroupLayout.PREFERRED_SIZE, 272, javax.swing.GroupLayout.PREFERRED_SIZE)))
                .addContainerGap())
        );
    ...
    private void txtcodigoKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtcodigoKeyPressed
        if (evt.getKeyChar() == KeyEvent.VK_ENTER) {
            try {
                ver_conex conn = new ver_conex();
                conn.sentencia = conn.conexion.createStatement();
                conn.resultado = conn.sentencia.executeQuery("SELECT cob_codigo,CONCAT(cli_nomape,' - ',obr_proyec) AS obra,cob_monto,cob_horfec FROM cobros,cliente,obras WHERE obras.cli_codigo = cliente.cli_codigo AND cobros.obr_codigo = obras.obr_codigo AND cob_estado LIKE 'ACTIVO' ORDER BY cob_codigo DESC");
                modelo3.setRowCount(0);
                ...
                buscadorentregas.setTitle("COBROS DE HONORARIOS");
    ...
    private void grillaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_grillaMouseClicked
        try {
            ...
            ver_conex conn = new ver_conex();
            conn.sentencia = conn.conexion.createStatement();
            conn.resultado = conn.sentencia.executeQuery("SELECT IFNULL(SUM(cob_monto),0) AS PAGADO FROM cobros WHERE obr_codigo = " + codigo + " AND cob_estado LIKE 'ACTIVO'");
            conn.resultado.next();
            txtmontoacobrar1.setText(conn.resultado.getString("PAGADO"));
    ...
    private void grilla4MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_grilla4MouseClicked
        try {
            ...
            int mensaje = JOptionPane.showConfirmDialog(this, "Deseas Anular Cobro N° --> " + txtcodigo.getText(), "Confirmar", JOptionPane.YES_NO_OPTION);
            if (mensaje == JOptionPane.YES_OPTION)//si quiere borrar
            {
                borrar();
            } else // no quiere borrar
