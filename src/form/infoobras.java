...
public class infoobras extends javax.swing.JPanel {
    ...
    public infoobras() throws ClassNotFoundException, InstantiationException, IllegalAccessException, SQLException {
        initComponents();
        cargacliente();
        des_op();
        des_btn();
    ...
    public void gen_reporte() throws JRException {
        try {
            if (op1.isSelected()) {
                sentencia = "SELECT obras.`obr_codigo` AS obras_obr_codigo,cliente.`cli_nomape` AS cliente_cli_nomape,obras.`obr_fecha` AS obras_obr_fecha,obras.`obr_proyec` AS obras_obr_proyec,obras.`obr_presup` AS obras_obr_presup,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `usuarios` usuarios INNER JOIN `obras` obras ON usuarios.`usu_codigo` = obras.`usu_codigo`,`cliente` cliente WHERE obras.cli_codigo = cliente.cli_codigo AND obr_estado LIKE 'ACTIVO' ORDER BY obr_codigo DESC";
            } else if (op3.isSelected()) {
                sentencia = "SELECT obras.`obr_codigo` AS obras_obr_codigo,cliente.`cli_nomape` AS cliente_cli_nomape,obras.`obr_fecha` AS obras_obr_fecha,obras.`obr_proyec` AS obras_obr_proyec,obras.`obr_presup` AS obras_obr_presup,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `usuarios` usuarios INNER JOIN `obras` obras ON usuarios.`usu_codigo` = obras.`usu_codigo`,`cliente` cliente WHERE obras.cli_codigo = cliente.cli_codigo AND obr_estado LIKE 'ACTIVO' AND LEFT(`obr_fecha`,10) BETWEEN  '" + txtfdesde.getText() + "' AND '" + txtfhasta.getText() + "' ORDER BY obr_codigo DESC";
            } else if (op4.isSelected()) {
                sentencia = "SELECT obras.`obr_codigo` AS obras_obr_codigo,cliente.`cli_nomape` AS cliente_cli_nomape,obras.`obr_fecha` AS obras_obr_fecha,obras.`obr_proyec` AS obras_obr_proyec,obras.`obr_presup` AS obras_obr_presup,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `usuarios` usuarios INNER JOIN `obras` obras ON usuarios.`usu_codigo` = obras.`usu_codigo`,`cliente` cliente WHERE obras.cli_codigo = cliente.cli_codigo AND obras.cli_codigo = " + cliente + " AND obr_estado LIKE 'ACTIVO' ORDER BY obr_codigo DESC";
            } else if (op8.isSelected()) {
                sentencia = "SELECT obras.`obr_codigo` AS obras_obr_codigo,cliente.`cli_nomape` AS cliente_cli_nomape,obras.`obr_fecha` AS obras_obr_fecha,obras.`obr_proyec` AS obras_obr_proyec,obras.`obr_presup` AS obras_obr_presup,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `usuarios` usuarios INNER JOIN `obras` obras ON usuarios.`usu_codigo` = obras.`usu_codigo`,`cliente` cliente WHERE obras.cli_codigo = cliente.cli_codigo AND obr_estado LIKE 'ANULADO' ORDER BY obr_codigo DESC";
            }
    ...
    private void initComponents() {
        ...
        op5.setText("Informe Semanal de Obras");
    ...
    private void btngenerarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btngenerarActionPerformed
        try {
            ...
            Logger.getLogger(infoobras.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void gen_reporte2() throws JRException {
        ...
        URL urlMaestro = getClass().getClassLoader().getResource("repo/robratotal.jasper");
        ///////////////////////////////// Cargamos el reporte
        des_op();
        des_btn();
        lim_text();
     ...
     private void gen_reporte3() throws JRException {
        /////////////////////////////////instanciamos
        ver_conex conn = new ver_conex();
        HashMap parameters = new HashMap();
        ///////////////////////////////// preparamos parametros del reporte
        parameters.put("fdesde", txtfdesde.getText());
        parameters.put("fhasta", txtfhasta.getText());
        parameters.put("SUBREPORT_DIR", "\\ambiente\\src\\repo\\");
        ///////////////////////////////// patch de los reporetes
        URL urlMaestro = getClass().getClassLoader().getResource("repo/rsemanal.jasper");
        ///////////////////////////////// Cargamos el reporte
        des_op();
        des_btn();
        lim_text();
