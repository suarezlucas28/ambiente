...
public class infodepositos extends javax.swing.JPanel {
    ...
    public infodepositos() throws ClassNotFoundException, InstantiationException, IllegalAccessException, SQLException {
        initComponents();
        cargacliente();
        des_op();
        des_btn();
    ...
    try {
    if (op1.isSelected()){
    sentencia = "SELECT movimientos.`nro_recibo` AS movimientos_nro_recibo,movimientos.`cue_monto` AS movimientos_cue_monto,movimientos.`cue_fechor` AS movimientos_cue_fechor,cliente.`cli_nomape` AS cliente_cli_nomape,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `obras` obras INNER JOIN `movimientos` movimientos ON obras.`obr_codigo` = movimientos.`obr_codigo`INNER JOIN `usuarios` usuarios ON movimientos.`usu_codigo` = usuarios.`usu_codigo` AND usuarios.`usu_codigo` = obras.`usu_codigo`,`cliente` cliente WHERE obras.cli_codigo = cliente.cli_codigo AND cue_opera LIKE 'ENTREGA' AND cue_estado LIKE 'ACTIVO' ORDER BY nro_recibo DESC";    
    } else if (op2.isSelected()){
    sentencia = "SELECT movimientos.`nro_recibo` AS movimientos_nro_recibo,movimientos.`cue_monto` AS movimientos_cue_monto,movimientos.`cue_fechor` AS movimientos_cue_fechor,cliente.`cli_nomape` AS cliente_cli_nomape,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `obras` obras INNER JOIN `movimientos` movimientos ON obras.`obr_codigo` = movimientos.`obr_codigo`INNER JOIN `usuarios` usuarios ON movimientos.`usu_codigo` = usuarios.`usu_codigo` AND usuarios.`usu_codigo` = obras.`usu_codigo`,`cliente` cliente WHERE obras.cli_codigo = cliente.cli_codigo AND cue_opera LIKE 'ENTREGA' AND `nro_recibo` = "+txtcodigo.getText();
    } else if (op3.isSelected()){
    sentencia = "SELECT movimientos.`nro_recibo` AS movimientos_nro_recibo,movimientos.`cue_monto` AS movimientos_cue_monto,movimientos.`cue_fechor` AS movimientos_cue_fechor,cliente.`cli_nomape` AS cliente_cli_nomape,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `obras` obras INNER JOIN `movimientos` movimientos ON obras.`obr_codigo` = movimientos.`obr_codigo`INNER JOIN `usuarios` usuarios ON movimientos.`usu_codigo` = usuarios.`usu_codigo` AND usuarios.`usu_codigo` = obras.`usu_codigo`,`cliente` cliente WHERE obras.cli_codigo = cliente.cli_codigo AND cue_opera LIKE 'ENTREGA' AND cue_estado LIKE 'ACTIVO' AND LEFT(`cue_fechor`,10) BETWEEN  '"+txtfdesde.getText()+"' AND '"+txtfhasta.getText()+"' ORDER BY nro_recibo DESC";    
    } else if (op4.isSelected()){
    sentencia = "SELECT movimientos.`nro_recibo` AS movimientos_nro_recibo,movimientos.`cue_monto` AS movimientos_cue_monto,movimientos.`cue_fechor` AS movimientos_cue_fechor,cliente.`cli_nomape` AS cliente_cli_nomape,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `obras` obras INNER JOIN `movimientos` movimientos ON obras.`obr_codigo` = movimientos.`obr_codigo`INNER JOIN `usuarios` usuarios ON movimientos.`usu_codigo` = usuarios.`usu_codigo` AND usuarios.`usu_codigo` = obras.`usu_codigo`,`cliente` cliente WHERE obras.cli_codigo = cliente.cli_codigo AND cue_opera LIKE 'ENTREGA' AND cue_estado LIKE 'ACTIVO' AND `obras`.`cli_codigo` = "+cliente+" ORDER BY nro_recibo DESC";    
    } else if (op8.isSelected()){
    sentencia = "SELECT movimientos.`nro_recibo` AS movimientos_nro_recibo,movimientos.`cue_monto` AS movimientos_cue_monto,movimientos.`cue_fechor` AS movimientos_cue_fechor,cliente.`cli_nomape` AS cliente_cli_nomape,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `obras` obras INNER JOIN `movimientos` movimientos ON obras.`obr_codigo` = movimientos.`obr_codigo`INNER JOIN `usuarios` usuarios ON movimientos.`usu_codigo` = usuarios.`usu_codigo` AND usuarios.`usu_codigo` = obras.`usu_codigo`,`cliente` cliente WHERE obras.cli_codigo = cliente.cli_codigo AND cue_opera LIKE 'ENTREGA' AND cue_estado LIKE 'ANULADO' ORDER BY nro_recibo DESC";    
...
op1.requestFocus();
    ...
    private void btngenerarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btngenerarActionPerformed
        try {
            ...
            Logger.getLogger(infodepositos.class.getName()).log(Level.SEVERE, null, ex);
