...
public class infocobros extends javax.swing.JPanel {
    ...
    public infocobros() throws ClassNotFoundException, InstantiationException, IllegalAccessException, SQLException {
        initComponents();
        cargacliente();
        des_op();
        des_btn();
    ...
    String SQL = "select * from vobras";
    ver_conex  conn = new ver_conex();
    conn.sentencia = conn.conexion.createStatement();
    conn.resultado = conn.sentencia.executeQuery(SQL);
    cbo_cliente.removeAllItems();
    while (conn.resultado.next()) {
    cbo_cliente.addItem(conn.resultado.getString("obra"));
    }
    }
    ...
    try {
    if (op1.isSelected()){
    sentencia = "SELECT cobros.`cob_codigo` ,CONCAT(cliente.`cli_nomape`,' - ',obras.`obr_proyec`) AS obra,cobros.`cob_horfec`,cobros.`cob_monto`,cobros.`cob_observ`,cobros.`cob_fecrec`,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `usuarios` usuarios INNER JOIN `cobros` cobros ON usuarios.`usu_codigo` = cobros.`usu_codigo` INNER JOIN `obras` obras ON usuarios.`usu_codigo` = obras.`usu_codigo` AND obras.`obr_codigo` = cobros.`obr_codigo`,`cliente` cliente WHERE cliente.cli_codigo = obras.cli_codigo AND cob_estado LIKE 'ACTIVO' ORDER BY cob_codigo DESC";    
    } else if (op2.isSelected()){
    sentencia = "SELECT cobros.`cob_codigo` ,CONCAT(cliente.`cli_nomape`,' - ',obras.`obr_proyec`) AS obra,cobros.`cob_horfec`,cobros.`cob_monto`,cobros.`cob_observ`,cobros.`cob_fecrec`,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `usuarios` usuarios INNER JOIN `cobros` cobros ON usuarios.`usu_codigo` = cobros.`usu_codigo` INNER JOIN `obras` obras ON usuarios.`usu_codigo` = obras.`usu_codigo` AND obras.`obr_codigo` = cobros.`obr_codigo`,`cliente` cliente WHERE cliente.cli_codigo = obras.cli_codigo AND cob_estado LIKE 'ACTIVO' AND `cob_codigo` = "+txtcodigo.getText();
    } else if (op3.isSelected()){
    sentencia = "SELECT cobros.`cob_codigo` ,CONCAT(cliente.`cli_nomape`,' - ',obras.`obr_proyec`) AS obra,cobros.`cob_horfec`,cobros.`cob_monto`,cobros.`cob_observ`,cobros.`cob_fecrec`,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `usuarios` usuarios INNER JOIN `cobros` cobros ON usuarios.`usu_codigo` = cobros.`usu_codigo` INNER JOIN `obras` obras ON usuarios.`usu_codigo` = obras.`usu_codigo` AND obras.`obr_codigo` = cobros.`obr_codigo`,`cliente` cliente WHERE cliente.cli_codigo = obras.cli_codigo AND cob_estado LIKE 'ACTIVO' AND LEFT(`cob_horfec`,10) BETWEEN  '"+txtfdesde.getText()+"' AND '"+txtfhasta.getText()+"' ORDER BY cob_codigo DESC";    
    } else if (op4.isSelected()){
    sentencia = "SELECT cobros.`cob_codigo` ,CONCAT(cliente.`cli_nomape`,' - ',obras.`obr_proyec`) AS obra,cobros.`cob_horfec`,cobros.`cob_monto`,cobros.`cob_observ`,cobros.`cob_fecrec`,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `usuarios` usuarios INNER JOIN `cobros` cobros ON usuarios.`usu_codigo` = cobros.`usu_codigo` INNER JOIN `obras` obras ON usuarios.`usu_codigo` = obras.`usu_codigo` AND obras.`obr_codigo` = cobros.`obr_codigo`,`cliente` cliente WHERE cliente.cli_codigo = obras.cli_codigo AND cob_estado LIKE 'ACTIVO' AND `cobros`.`obr_codigo` = "+cliente+" ORDER BY cob_codigo DESC";    
    } else if (op8.isSelected()){
    sentencia = "SELECT cobros.`cob_codigo` ,CONCAT(cliente.`cli_nomape`,' - ',obras.`obr_proyec`) AS obra,cobros.`cob_horfec`,cobros.`cob_monto`,cobros.`cob_observ`,cobros.`cob_fecrec`,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `usuarios` usuarios INNER JOIN `cobros` cobros ON usuarios.`usu_codigo` = cobros.`usu_codigo` INNER JOIN `obras` obras ON usuarios.`usu_codigo` = obras.`usu_codigo` AND obras.`obr_codigo` = cobros.`obr_codigo`,`cliente` cliente WHERE cliente.cli_codigo = obras.cli_codigo AND cob_estado LIKE 'ANULADO' ORDER BY cob_codigo DESC";    
    ...
    String SQL = "SELECT * FROM vobras WHERE obra ='" + a +"'";
    ver_conex  conn = new ver_conex();
    conn.sentencia = conn.conexion.createStatement();
    conn.resultado = conn.sentencia.executeQuery(SQL);
    conn.resultado.next();
    return(conn.resultado.getInt("obr_codigo"));
    }   
...
op1.requestFocus();
    ...
    private void btngenerarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btngenerarActionPerformed
        try {
            ...
            Logger.getLogger(infocobros.class.getName()).log(Level.SEVERE, null, ex);
