...
public class infocompras extends javax.swing.JPanel {
    ...
    public infocompras() throws ClassNotFoundException, InstantiationException, IllegalAccessException, SQLException {
        initComponents();
        cargacliente();
        des_op();
        des_btn();
    ...
    public void gen_reporte() throws JRException {
        try {
            if (op1.isSelected()) {
                sentencia = "SELECT compras.`com_codigo` AS compras_com_codigo,proveedor.`pro_nombre` AS proveedor_pro_nombre,CONCAT(cliente.`cli_nomape`,' - ',obras.`obr_proyec`) AS obras_obr_proyec,compras.`com_fechor` AS compras_com_fechor,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `usuarios` usuarios INNER JOIN `compras` compras ON usuarios.`usu_codigo` = compras.`usu_codigo` INNER JOIN `obras` obras ON usuarios.`usu_codigo` = obras.`usu_codigo` AND obras.`obr_codigo` = compras.`obr_codigo` INNER JOIN `proveedor` proveedor ON compras.`pro_codigo` = proveedor.`pro_codigo`,`cliente` cliente WHERE obras.cli_codigo = cliente.cli_codigo AND com_estado LIKE 'ACTIVO' ORDER BY com_codigo DESC";
            } else if (op2.isSelected()) {
                sentencia = "SELECT compras.`com_codigo` AS compras_com_codigo,proveedor.`pro_nombre` AS proveedor_pro_nombre,CONCAT(cliente.`cli_nomape`,' - ',obras.`obr_proyec`) AS obras_obr_proyec,compras.`com_fechor` AS compras_com_fechor,usuarios.`usu_usuari` AS usuarios_usu_usuari,compras_detalle.ite_codigo,ite_descri,uni_descri,com_cantid FROM `usuarios` usuarios INNER JOIN `compras` compras ON usuarios.`usu_codigo` = compras.`usu_codigo` INNER JOIN `obras` obras ON usuarios.`usu_codigo` = obras.`usu_codigo` AND obras.`obr_codigo` = compras.`obr_codigo` INNER JOIN `proveedor` proveedor ON compras.`pro_codigo` = proveedor.`pro_codigo`, `cliente` cliente,unidades,items,compras_detalle WHERE obras.cli_codigo = cliente.cli_codigo AND compras.com_codigo = compras_detalle.com_codigo AND compras_detalle.ite_codigo = items.ite_codigo AND items.uni_precom = unidades.uni_codigo and compras.com_codigo = " + txtcodigo.getText();
            } else if (op3.isSelected()) {
                sentencia = "SELECT compras.`com_codigo` AS compras_com_codigo,proveedor.`pro_nombre` AS proveedor_pro_nombre,CONCAT(cliente.`cli_nomape`,' - ',obras.`obr_proyec`) AS obras_obr_proyec,compras.`com_fechor` AS compras_com_fechor,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `usuarios` usuarios INNER JOIN `compras` compras ON usuarios.`usu_codigo` = compras.`usu_codigo` INNER JOIN `obras` obras ON usuarios.`usu_codigo` = obras.`usu_codigo` AND obras.`obr_codigo` = compras.`obr_codigo` INNER JOIN `proveedor` proveedor ON compras.`pro_codigo` = proveedor.`pro_codigo`,`cliente` cliente WHERE obras.cli_codigo = cliente.cli_codigo AND com_estado LIKE 'ACTIVO' AND LEFT(`com_fechor`,10) BETWEEN  '" + txtfdesde.getText() + "' AND '" + txtfhasta.getText() + "' ORDER BY com_codigo DESC";
            } else if (op4.isSelected()) {
                sentencia = "SELECT compras.`com_codigo` AS compras_com_codigo,proveedor.`pro_nombre` AS proveedor_pro_nombre,CONCAT(cliente.`cli_nomape`,' - ',obras.`obr_proyec`) AS obras_obr_proyec,compras.`com_fechor` AS compras_com_fechor,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `usuarios` usuarios INNER JOIN `compras` compras ON usuarios.`usu_codigo` = compras.`usu_codigo` INNER JOIN `obras` obras ON usuarios.`usu_codigo` = obras.`usu_codigo` AND obras.`obr_codigo` = compras.`obr_codigo` INNER JOIN `proveedor` proveedor ON compras.`pro_codigo` = proveedor.`pro_codigo`,`cliente` cliente WHERE obras.cli_codigo = cliente.cli_codigo AND com_estado LIKE 'ACTIVO' AND `compras`.`pro_codigo` = " + cliente + " ORDER BY com_codigo DESC";
            ...
            } else if (op7.isSelected()) {
                sentencia = "SELECT compras.`com_codigo` AS compras_com_codigo,proveedor.`pro_nombre` AS proveedor_pro_nombre,CONCAT(cliente.`cli_nomape`,' - ',obras.`obr_proyec`) AS obras_obr_proyec,compras.`com_fechor` AS compras_com_fechor,usuarios.`usu_usuari` AS usuarios_usu_usuari FROM `usuarios` usuarios INNER JOIN `compras` compras ON usuarios.`usu_codigo` = compras.`usu_codigo` INNER JOIN `obras` obras ON usuarios.`usu_codigo` = obras.`usu_codigo` AND obras.`obr_codigo` = compras.`obr_codigo` INNER JOIN `proveedor` proveedor ON compras.`pro_codigo` = proveedor.`pro_codigo`,`cliente` cliente WHERE obras.cli_codigo = cliente.cli_codigo AND com_estado LIKE 'ACTIVO' AND `compras`.`obr_codigo` = " + txtcodigo.getText() + " ORDER BY com_codigo DESC";
    ...
    private void gen_reporte2() throws JRException {
        ...
        URL urlMaestro = getClass().getClassLoader().getResource("repo/rpcanmatutiycom.jasper");
        ///////////////////////////////// Cargamos el reporte
        des_op();
        des_btn();
        lim_text();
    ...
    private void initComponents() {
        ...
        op7.setFont(new java.awt.Font("Arial", 1, 12)); // NOI18N
        op7.setText("Obra");
        ...
        op7.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                op7KeyPressed(evt);
            }
        });
        ...
        op5.setText("Materiales Comprados y Retirados");
        ...
        jLabel6.setText("Selecione una opción para desplegar el informe:");
    ...
    private void btngenerarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btngenerarActionPerformed
        try {
            ...
            Logger.getLogger(infocompras.class.getName()).log(Level.SEVERE, null, ex);
...
op1.setEnabled(false);
    ...
    private void op7KeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_op7KeyPressed
        // TODO add your handling code here:
    }//GEN-LAST:event_op7KeyPressed
