...
private void gencod(){
	...
	ver_conex  conn = new ver_conex();
        ...
        conn.resultado = conn.sentencia.executeQuery("SELECT IFNULL(MAX(pro_codigo),0)+1 AS xxx FROM proveedor");
        conn.resultado.next();
...
private void cargagrilla() throws SQLException{
    ver_conex  conn = new ver_conex();
    conn.sentencia = conn.conexion.createStatement();
    conn.resultado = conn.sentencia.executeQuery("SELECT pro_codigo,pro_nombre from proveedor order by pro_nombre");
...
private void operaciones (){
        try {
            String SQL = "call abm_proveedor("+operacion+","+txtcodigo.getText()+",'"+txtnombre.getText()+"','"+txttelefono.getText()+"','"+txtdireccion.getText()+"')";
            ...
            JOptionPane.showMessageDialog(null, "Se ha producido un error en la operación o bien esta intentando borrar un Proveedor en uso", "", JOptionPane.ERROR_MESSAGE);
...
txtdireccion.setEnabled(false);
    ...
    private void initComponents() {
        ...
        setMaximumSize(new java.awt.Dimension(505, 230));
        setMinimumSize(new java.awt.Dimension(505, 230));
        setPreferredSize(new java.awt.Dimension(505, 230));
    ...
    private void txtnombreActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_txtnombreActionPerformed
        ...
        if (campo.isEmpty()) {
            ...
            if (operacion == 1){
                ...
                ver_conex conn = new ver_conex();//instanciamos
                conn.sentencia = conn.conexion.createStatement();
                conn.resultado = conn.sentencia.executeQuery("SELECT `pro_nombre` FROM `proveedor` WHERE `pro_nombre` = '" + txtnombre.getText() + "'");
                boolean encontro = conn.resultado.next();
                if (encontro == true)//encontro
                {
                    JOptionPane.showMessageDialog(null, "Ya existe Proveedor con este Nombre!: " + txtnombre.getText(), "Atención", JOptionPane.ERROR_MESSAGE);
 ...
 evt.consume();
    ...
    private void grillaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_grillaMouseClicked
        try {
            ...
            ver_conex conn = new ver_conex();//instanciamos
            conn.sentencia = conn.conexion.createStatement();
            conn.resultado = conn.sentencia.executeQuery("SELECT * FROM `proveedor` WHERE `pro_codigo` = " + codigo);
            conn.resultado.next();
            txttelefono.setText(conn.resultado.getString("pro_telefo"));
            txtdireccion.setText(conn.resultado.getString("pro_domici"));
            if(operacion == 3){
            txtnombre.setEnabled(true);
            txtnombre.requestFocus();
            grilla.setEnabled(false);
...
if (operacion != 0){txttelefono.setEnabled(true);
txtnombre.setEnabled(false);
txtdireccion.setEnabled(false);
txttelefono.requestFocus();
}
