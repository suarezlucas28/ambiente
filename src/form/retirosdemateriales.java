 ...
 * @author Luke
 */
public class retirosdemateriales extends javax.swing.JPanel {
...
@Override
    ...
    public retirosdemateriales() throws ClassNotFoundException, InstantiationException, IllegalAccessException, SQLException {
        initComponents();
    ...
    private void gencod() {
        try {
            ...
            conn.resultado = conn.sentencia.executeQuery("SELECT IFNULL(MAX(ret_codigo),0)+1 AS xxx FROM retiros");
            conn.resultado.next();
    ...
    private void CabObra() {
        modelo1.addColumn("CÓD");
        modelo1.addColumn("OBRA");
        grilla1.getColumnModel().getColumn(0).setPreferredWidth(5);
    ...
    public void grilla3() {
        ...
        try {
            ver_conex conn2 = new ver_conex();//instanciamos
            snt = conn2.conexion.createStatement();
            recur1 = snt.executeQuery("SELECT ret_codigo,pro_nombre,CONCAT(cli_nomape,' - ',obr_proyec),ret_fechor FROM retiros,obras,proveedor,cliente WHERE retiros.pro_codigo = proveedor.pro_codigo AND retiros.obr_codigo = obras.obr_codigo AND obras.cli_codigo = cliente.cli_codigo AND CONCAT(cli_nomape,' - ',obr_proyec) LIKE '%" + wqqqq + "%' AND ret_estado LIKE 'ACTIVO' ORDER BY ret_codigo DESC");
            System.out.println(recur1);
            if (recur1.wasNull()) {
                ab = 0;
                return;
            ...
            if (recur1.next()) {
                do {
                    ...
                    ad = recur1.getString("CONCAT(cli_nomape,' - ',obr_proyec)");
    ...
    private void agregar() throws SQLException {
        Fecha("Fecha");
        String SQL = "INSERT INTO retiros (ret_codigo,pro_codigo,obr_codigo,usu_codigo,ret_fechor,ret_monto) "
        ...
        for (int i = 0; i < model.getRowCount(); i++) {
            ...
            if (value != null && value) {
                ...
                String SQL1 = "INSERT INTO retiros_detalle_material (ret_codigo,ite_codigo,rdm_cantid,rdm_subtot,rdm_unitari) VALUES (" + lblcodigo.getText() + "," + codigo + "," + cantidad + "," + cantcon + "," + costo + ")";
                conn.sentencia = conn.conexion.createStatement();
                ...
                String cantidad = modelo.getValueAt(i, 4).toString();
                String cantcon = modelo.getValueAt(i, 5).toString();
                float auxfloat = Float.parseFloat(cantcon);
                apagar = apagar + auxfloat;
                String SQL1 = "INSERT INTO retiros_detalle_material (ret_codigo,ite_codigo,rdm_cantid,rdm_subtot,rdm_unitari,rdm_pagado) VALUES (" + lblcodigo.getText() + "," + codigo + "," + cantidad + "," + cantcon + "," + costo + ",'NO')";
                conn.sentencia = conn.conexion.createStatement();
                conn.sentencia.executeUpdate(SQL1);
                conn.sentencia.close();
                c++;
        ...
        String SQL1 = "UPDATE retiros SET ret_apagar = " + apagar + " WHERE ret_codigo = " + lblcodigo.getText();
//        String SQL2 = "UPDATE movimientos SET cue_monto = " + txttotal.getText() + ",cue_fechor = '" + fechahora + "' WHERE cue_segun = 'ENTREGA " + lblcodigo.getText() + "'";
    ...
    private void modificar() throws SQLException {
        Fecha("Fecha");
        String SQL = "UPDATE retiros SET pro_codigo = " + proveedor + ",obr_codigo = " + txt_item_cod.getText() + ",usu_codigo = " + acceso.usuario + ",ret_monto = " + txttotal.getText() + ",ret_apagar = " + apagar + " WHERE ret_codigo = " + lblcodigo.getText();
//        String SQL2 = "UPDATE movimientos SET cue_monto = " + txttotal.getText() + ",cue_fechor = '" + fechahora + "' WHERE cue_segun = 'ENTREGA " + lblcodigo.getText() + "'";
        System.out.println(SQL);
        ver_conex conn = new ver_conex();//instanciamos
        conn.sentencia = conn.conexion.createStatement();
...
//        conn.sentencia.executeUpdate(SQL2);//OJO LE PASO LA SENTENCIA
        String SQL1 = "DELETE FROM retiros_detalle_material WHERE ret_codigo =  " + lblcodigo.getText();
        conn.sentencia = conn.conexion.createStatement();
        conn.sentencia.executeUpdate(SQL1);
        try {
            javax.swing.table.TableModel model = grilla.getModel();
            ...
            if (value != null && value) {
                ...
                String cantidad = modelo.getValueAt(i, 4).toString();
                String cantcon = modelo.getValueAt(i, 5).toString();
                String SQL2 = "INSERT INTO retiros_detalle_material (ret_codigo,ite_codigo,rdm_cantid,rdm_subtot,rdm_unitari) VALUES (" + lblcodigo.getText() + "," + codigo + "," + cantidad + "," + cantcon + "," + costo + ")";
                conn.sentencia = conn.conexion.createStatement();
                ...
                float auxfloat = Float.parseFloat(cantcon);
                apagar = apagar + auxfloat;
                String SQL2 = "INSERT INTO retiros_detalle_material (ret_codigo,ite_codigo,rdm_cantid,rdm_subtot,rdm_unitari,rdm_pagado) VALUES (" + lblcodigo.getText() + "," + codigo + "," + cantidad + "," + cantcon + "," + costo + ",'NO')";
                conn.sentencia = conn.conexion.createStatement();
                conn.sentencia.executeUpdate(SQL2);
                conn.sentencia.close();
                c++;
            ...
            String SQL3 = "UPDATE retiros SET ret_apagar = " + apagar + " WHERE ret_codigo = " + lblcodigo.getText();
//        String SQL2 = "UPDATE movimientos SET cue_monto = " + txttotal.getText() + ",cue_fechor = '" + fechahora + "' WHERE cue_segun = 'ENTREGA " + lblcodigo.getText() + "'";
        System.out.println(SQL3);
        conn.sentencia = conn.conexion.createStatement();
        conn.sentencia.executeUpdate(SQL3);
    ...
    private void borrar() throws SQLException {
        String SQL = "UPDATE retiros SET ret_estado = 'ANULADO' WHERE ret_codigo = " + lblcodigo.getText();
...
//        conn.sentencia.executeUpdate(SQL2);
    ...
    private void cargadatos() throws SQLException {
        ver_conex conn = new ver_conex();
        conn.sentencia = conn.conexion.createStatement();
        conn.resultado = conn.sentencia.executeQuery("SELECT * FROM retiros WHERE ret_codigo = " + lblcodigo.getText());
        conn.resultado.next();
        ...
        int obra = conn.resultado.getInt("obr_codigo");
        txt_item_cod.setText(String.valueOf(obra));
        ...
        conn.resultado = conn.sentencia.executeQuery("SELECT obr_codigo,CONCAT(cli_nomape,' - ',obr_proyec) FROM obras,cliente WHERE obras.cli_codigo = cliente.cli_codigo AND obr_codigo = " + txt_item_cod.getText());
        conn.resultado.next();
        lbldescri.setText(conn.resultado.getString("CONCAT(cli_nomape,' - ',obr_proyec)"));
        conn.sentencia = conn.conexion.createStatement();
        conn.resultado = conn.sentencia.executeQuery("SELECT retiros_detalle_material.ite_codigo,ite_descri,uni_descri,rdm_unitari,rdm_cantid,rdm_subtot,rdm_pagado FROM unidades,retiros_detalle_material,items WHERE items.ite_codigo = retiros_detalle_material.ite_codigo AND items.uni_precom = unidades.uni_codigo AND ret_codigo = " + lblcodigo.getText());
    ...
    private void imprimir() throws SQLException, JRException {
        try {
            ...
            String sentencia = "SELECT retiros.ret_codigo,DAY(`ret_fechor`) AS dia, MONTH(`ret_fechor`) AS mes, YEAR(`ret_fechor`) AS anno,pro_nombre,CONCAT(cli_nomape,' - ',obr_proyec),rdm_cantid,ite_descri,rdm_pagado,rdm_unitari,rdm_subtot FROM items,retiros,obras,cliente,proveedor,retiros_detalle_material WHERE retiros.pro_codigo = proveedor.pro_codigo AND retiros.obr_codigo = obras.obr_codigo AND obras.cli_codigo = cliente.cli_codigo AND retiros_detalle_material.ret_codigo = retiros.ret_codigo AND retiros_detalle_material.ite_codigo = items.ite_codigo AND retiros.ret_codigo = " + lblcodigo.getText();
            ////////////////////////////////instanciamos
        ...
        ////////////////////////////// preapra el patch
        URL urlMaestro = getClass().getClassLoader().getResource("repo/entregademateriales.jasper");
        ////////////////////////////// Cargamos el reporte
                    ...
                    .addGroup(layout.createSequentialGroup()
                        ...
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(layout.createSequentialGroup()
                                ...
                                .addGap(10, 10, 10)
                                .addComponent(lbldescri, javax.swing.GroupLayout.PREFERRED_SIZE, 268, javax.swing.GroupLayout.PREFERRED_SIZE))
                            .addGroup(layout.createSequentialGroup()
...
//            operaciones();
            ...
            {
            int mensaje1 = JOptionPane.showConfirmDialog(this, "LA ORDEN SE IMPRIMIRÁ POR DUPLICADO, DESEA QUE SEA POR TRIPLICADO?", "CONFIRMA", JOptionPane.YES_NO_OPTION);
            if (mensaje1 == JOptionPane.YES_OPTION)//si quiere borrar
            {
                try {
        imprimir();
        imprimir();
        ...
        Logger.getLogger(presupuesto.class.getName()).log(Level.SEVERE, null, ex);
            ...
            Logger.getLogger(retirosdemateriales.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void lblcodigoKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_lblcodigoKeyPressed
        if (evt.getKeyChar() == KeyEvent.VK_ENTER) {
            try {
                ver_conex conn = new ver_conex();
                conn.sentencia = conn.conexion.createStatement();
                conn.resultado = conn.sentencia.executeQuery("SELECT ret_codigo,pro_nombre,CONCAT(cli_nomape,' - ',obr_proyec),ret_fechor FROM retiros,obras,proveedor,cliente WHERE retiros.pro_codigo = proveedor.pro_codigo AND retiros.obr_codigo = obras.obr_codigo AND obras.cli_codigo = cliente.cli_codigo AND ret_estado LIKE 'ACTIVO' ORDER BY ret_codigo DESC");
                modelo3.setRowCount(0);
    ...
    private void txt_item_codActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_txt_item_codActionPerformed
        ...
        if (campo.isEmpty()) {
            ...
            try {
                ver_conex conn = new ver_conex();
                conn.sentencia = conn.conexion.createStatement();
                conn.resultado = conn.sentencia.executeQuery("SELECT obras.obr_codigo,CONCAT(cli_nomape,' - ',obr_proyec) AS DETALLE FROM obras,cliente WHERE obras.cli_codigo = cliente.cli_codigo AND obr_estado LIKE 'ACTIVO' AND obras.obr_codigo = " + txt_item_cod.getText());
                ...
                if (encontro == true) {
                    ...
                    JOptionPane.showMessageDialog(null, "No se encuentra Obra con codigo -->" + txt_item_cod.getText(), "Atención", JOptionPane.ERROR_MESSAGE);
                    txt_item_cod1.setText("");
                    return;
                }
            } catch (SQLException ex) {
    ...
    private void txtbuscadorKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtbuscadorKeyReleased
        ...
        grilla();        // TODO add your handling code here:
...
String campo = txttotal.getText();
        if (campo.isEmpty()) {
            JOptionPane.showMessageDialog(this, "El campo está vacío, Cargue los datos correctamente");
            return;
        } else {
            txttotal.setText(String.valueOf(Redondear(Float.parseFloat(txttotal.getText()))));
            btngrabar.setEnabled(true);
...
txttotal.selectAll();// TODO add your handling code here:
